...
def _render_dashboard_body(hours: int) -> None:
    ...
    with middle_left:
        render_section("全服实时排行榜", "集中呈现近期表现突出的个人与队伍。")
    ...
    with pk_left:
        render_section("支部对抗榜", "按支部或单位汇总队伍成绩。")
        ...
        if branch_rows:
            ...
            st.info("暂无支部对抗成绩。")
    with pk_right:
        render_section("班级/单位排行预览", "按班级、单位或学习组织聚合呈现成绩。")
...
render_hero(
    title="数据大屏",
    subtitle="集中呈现答题热度、参与人数、全服实时榜、红军小队榜、支部对抗榜、班级或单位排行与实时战绩流。",
...
st.caption(
    f"统计窗口：最近 {hours} 小时 | 参与人数 {summary.get('recent_participant_count', 0)} | "
