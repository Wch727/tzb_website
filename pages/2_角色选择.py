...
render_hero(
    title="角色选择",
    subtitle="从不同身份进入长征征程，沿各自的任务线索展开剧情关卡与主线阅读。",
...
render_section("红军身份", "不同身份对应不同的任务重点、节点线索与成长路径。")
roles = list_roles()
role_cards = [
    {
...
with left:
    ...
    st.markdown(f"**身份线索：** {selected_role.get('special_hint', '')}")
...
with right:
    ...
    st.caption(f"参与信息：{st.session_state.get('user_name', '红色学习者')} | {st.session_state.get('unit_name', '体验组')}")
    ...
    if current_team:
        ...
        st.info("尚未编入红军小队；如需参加协作任务，可在活动中心完成编组。")
