...
讲解词要像红色纪念馆讲解员的现场口播：从展线、照片、地图、文物或复原场景进入，带观众一步一步看见历史现场。
""".strip()
...
要求：
1. 预计讲解时长约为 {duration}。
2. 正文控制在 650 到 900 字之间，宁可更完整，也不要过短。
3. 写成博物馆讲解员现场口吻，可以使用“各位观众”“请看这张地图”“请留意这个地点”等自然导览句。
4. 不要像论文摘要，也不要像机器模板；少用抽象判断，多写时间、地点、路线、人物动作、战场处境、会议现场和群众支前细节。
5. 结构要完整，但小标题不要机械堆叠；讲解应当自然衔接，像讲解员带观众沿展线前进。
6. 必须讲清：历史背景、关键经过、现场细节、历史意义、与前后节点的关系。
7. 内容必须建立在参考资料基础上，避免空泛抒情、避免堆砌口号、避免重复使用“不是……而是……”句式。
8. 如果资料有限，也要优先整合已有信息写成一篇可直接口播的完整讲解词。
...
3. 建议写成 4 到 5 段，适合节点详情页与互动展项展示。
4. 风格要像红色纪念馆讲解员：从地图、照片、地名、渡口、会议室、桥梁、山地等具体展陈对象进入。
5. 多写具体场景和细节，少写抽象套话；不要用“本节点体现了……”这类生硬表述开头。
6. 如果资料有限，也要优先整合已有事实形成完整讲解。
