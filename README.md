...
这是一个基于 `Streamlit + Python + Chroma` 的多页面红色教育互动平台，主题聚焦《长征史》与长征精神传播。项目已经按 **Streamlit Community Cloud 可部署** 的方式重构，主入口为 `app.py`。推送到 GitHub 后可以直接部署为一个固定网址，并通过后续 `git push` 自动更新线上版本。
...
- `pages/1_首页.py`：主展入口页
- `pages/3_长征路线.py`：长征路线选关大厅，按篇章选择节点
- `pages/14_节点展项.py`：单节点独立展项，展示图文讲解、人物、语音与互动入口
- `pages/4_剧情答题.py`：互动闯关大厅，角色选择、小队绑定、闭卷作答、答后解析都在这里完成
- `pages/5_知识库.py`：智能问答 + 图文知识卡片 + 错题复盘
- `pages/6_活动中心.py`：活动模板、分享链接、二维码、活动排行
- `pages/7_排行榜.py`：全局排行、活动排行、个人战绩
- `pages/8_配置页.py`：普通用户配置 + 管理员开放策略
- `pages/9_管理员后台.py`：活动管理、题库维护、导出、系统状态
- `pages/10_测试体验.py`：给组员快速试用
- `pages/11_讲解生成.py`：讲解稿 / 脚本生成
- `pages/12_数据大屏.py`：投屏展示用的数据大屏
- `pages/13_人物专题.py`：重要人物专题页

## 展览与闯关的使用路径

本版本把“展览学习”和“互动闯关”分开处理：

- 展览路径：从首页进入长征路线，选择篇章和节点，进入独立节点展项阅读图文讲解、人物专题和语音导览。
- 闯关路径：完成展项学习后，点击节点展项中的“进入闭卷闯关”，进入互动闯关大厅，选择身份后开始答题。
...
- 活动二维码：链接中包含 `activity_id`，扫码后进入对应活动。
- 小队二维码：链接中同时包含 `activity_id` 与 `team_id`，每支红军小队都有不同二维码，扫码后可识别对应小队。
- 线上部署后，二维码会使用 `config/system_settings.yaml` 中的 `public_base_url` 作为网址前缀。
- 如果更换 Streamlit 固定网址，请同步更新 `public_base_url`，再提交并 push 到 GitHub。

当前线上固定网址可配置为：

```yaml
public_base_url: https://tzbwebsite-hvpcfpjbbwetrrumyird28.streamlit.app
```

二维码依赖已写入 `requirements.txt`：

```text
qrcode[pil]
```

如果本地二维码不显示，请先执行 `pip install -r requirements.txt`。如果云端二维码不显示，请在 Community Cloud 重新部署并确认依赖安装日志没有报错。
...
- 不依赖额外单独启动的后端服务
- `.streamlit/config.toml` 已关闭默认侧边栏页面列表，避免把内部文件名直接暴露给参观者
